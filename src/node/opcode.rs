...
pub enum Instruction {
    ...
    ARETURN,
    RETURN,
    GETSTATIC(Node<GetStatic>),
    PUTSTATIC(Node<PutStatic>),
    GETFIELD(Node<GetField>),
    PUTFIELD(Node<PutField>),
    INVOKEVIRTUAL(Node<InvokeVirtual>),
    INVOKESPECIAL(Node<InvokeSpecial>),
    INVOKESTATIC(Node<InvokeStatic>),
    INVOKEINTERFACE(Node<InvokeInterface>),
    INVOKEDYNAMIC(Node<InvokeDynamic>),
    NEW(Node<New>),
    NEWARRAY(Node<ArrayType>),
    ANEWARRAY(Node<ANewArray>),
    ARRAYLENGTH,
    ATHROW,
    CHECKCAST(Node<CheckCast>),
    INSTANCEOF(Node<InstanceOf>),
    MONITORENTER,
    MONITOREXIT,
    WIDE(Node<Wide>),
    MULTIANEWARRAY(Node<MultiANewArray>),
    IFNULL(Node<i16>),
    IFNONNULL(Node<i16>),
    GOTO_W(Node<i64>),
    JSR_W(Node<i64>),
